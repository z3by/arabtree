...
async function main() {
    ...
    const judhamId = await prisma.lineageNode.findFirst({ where: { name: 'Judham' } }).then(n => n?.id);
    const makhzumId = await prisma.lineageNode.findFirst({ where: { name: 'Banu Makhzum' } }).then(n => n?.id);
    ...
    if (tayyId) {
        ...
        levantTribes.push({ name: 'Al-Sirhan', nameAr: 'السرحان', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: tayyId, childCount: 0, biography: 'A prominent tribe rooted in northern Jordan (Mafraq) and Al-Jawf, with historical ties to Tayy/Kalb.', biographyAr: 'قبيلة بارزة جذورها في شمال الأردن (المفرق) والجوف، ولها روابط تاريخية بطيء وكلب.', birthPlace: 'المفرق، الأردن', era: 'العصور الحديثة', latitude: 32.34, longitude: 36.20 });
        levantTribes.push({ name: 'Al-Mawali', nameAr: 'الموالي', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: tayyId, childCount: 0, biography: 'Historically dominant emirs of the Syrian steppe (Aleppo, Hama), claiming descent from Tayy through Fadl bin Rabi\'a.', biographyAr: 'أمراء البادية السورية (حلب وحماة) تاريخياً، ينتسبون إلى طيئ عبر فضل بن ربيعة.', birthPlace: 'حلب وحماة، سوريا', era: 'العصور الحديثة', latitude: 35.5, longitude: 37.5 });
        levantTribes.push({ name: 'Al-Khraishah', nameAr: 'الخريشا', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: tayyId, childCount: 0, biography: 'A preeminent sheikhly family leading the Ka\'abneh branch of the Bani Sakhr tribe in Jordan. Played major roles in Transjordanian history.', biographyAr: 'عشيرة مشيخة بارزة تقود فرع الكعابنة من قبيلة بني صخر في الأردن. لعبوا أدواراً رئيسية في تاريخ إمارة شرق الأردن.', birthPlace: 'الموقر، الأردن', era: 'العصور الحديثة', latitude: 31.8, longitude: 36.1 });
        levantTribes.push({ name: 'Al-Afadla', nameAr: 'العفادلة', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: tayyId, childCount: 0, biography: 'A prominent Arab tribe with deep historical roots in eastern Raqqa, Syria. Tracing back to Fadl ibn Rabi\'ah of Tayy.', biographyAr: 'قبيلة عربية بارزة ذات جذور تاريخية عميقة في شرق الرقة بسوريا. تعود في نسبها إلى فضل بن ربيعة من طيئ.', birthPlace: 'الرقة، سوريا', era: 'العصور الحديثة', latitude: 35.9, longitude: 39.0 });
        levantTribes.push({ name: 'Al-Fa\'our', nameAr: 'الفاعور', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: tayyId, childCount: 0, biography: 'The leading sheikhly family of the Al-Fadl tribes in the Syrian Golan Heights and beyond. Famous for their resistance against colonial powers.', biographyAr: 'عائلة المشيخة الرائدة لقبائل الفضل في الجولان السوري. اشتهروا بمقاومتهم للقوى الاستعمارية.', birthPlace: 'الجولان، سوريا', era: 'العصور الحديثة', latitude: 33.1, longitude: 35.8 });
        levantTribes.push({ name: 'Al-Sardiyeh', nameAr: 'السردية', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: tayyId, childCount: 0, biography: 'A historic and powerful Bedouin tribe in the Mafraq region. They were key members of the ancient Ahl Al-Shamal confederacy.', biographyAr: 'قبيلة بدوية ذات تاريخ قوي في المفرق الأردنية، وحلفاء رئيسيون في تحالف "أهل الشمال" التاريخي.', birthPlace: 'المفرق، الأردن', era: 'العصور الحديثة', latitude: 32.35, longitude: 36.35 });
        levantTribes.push({ name: 'Al-Azzamat', nameAr: 'العظامات', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: tayyId, childCount: 0, biography: 'A prominent tribe historically associated with the Ahl al-Jabal area spanning northern Jordan and southern Syria.', biographyAr: 'قبيلة بارزة ارتبطت تاريخياً بمنطقة أهل الجبل الممتدة بين شمال الأردن وجنوب سوريا.', birthPlace: 'الأردن - سوريا', era: 'العصور الحديثة', latitude: 32.4, longitude: 36.4 });
        levantTribes.push({ name: 'Al-Sharafat', nameAr: 'الشرفات', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: tayyId, childCount: 0, biography: 'Another major tribal group in the Badia (Mafraq/Southern Syria) closely tied to the Ahl al-Jabal confederation.', biographyAr: 'مجموعة قبلية كبرى أخرى في البادية (المفرق وجنوب سوريا) مرتبطة ارتباطاً وثيقاً بتحالف أهل الجبل.', birthPlace: 'المفرق، الأردن', era: 'العصور الحديثة', latitude: 32.38, longitude: 36.42 });
    }
    if (anazahId) {
        ...
        levantTribes.push({ name: 'Al-Fida\'an', nameAr: 'الفدعان', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: anazahId, childCount: 0, biography: 'A major and powerful branch of the \'Anazah confederation, largely situated in the Syrian desert and Euphrates region.', biographyAr: 'فرع رئيسي وقوي من قبيلة عنزة، يتواجد بكثافة في بادية الشام ومنطقة الفرات.', birthPlace: 'بادية الشام', era: 'العصور الحديثة', latitude: 34.5, longitude: 38.5 });
        levantTribes.push({ name: 'Al-Sba\'a', nameAr: 'السبعة', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: anazahId, childCount: 0, biography: 'Another highly prominent nomad branch of \'Anazah located in the Syrian interior.', biographyAr: 'فرع بدوي بارز آخر من قبيلة عنزة يتمركز في الداخل السوري.', birthPlace: 'بادية الشام', era: 'العصور الحديثة', latitude: 34.2, longitude: 38.2 });
    }
    if (tamimId) {
    ...
    if (hashim.id) {
        ...
        levantTribes.push({ name: 'Al-Zawahra', nameAr: 'الزواهرة', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 21, parentId: hashim.id, childCount: 0, biography: 'A massive clan within the Bani Hassan confederation, heavily centered in Zarqa, Jordan.', biographyAr: 'عشيرة ضخمة ضمن تحالف قبيلة بني حسن، تتركز بكثافة في محافظة الزرقاء بالأردن.', birthPlace: 'الزرقاء، الأردن', era: 'العصور الحديثة', latitude: 32.05, longitude: 36.09 });
    ...
    if (alHusayn.id) {
        levantTribes.push({ name: 'Al-Nu\'aym', nameAr: 'النعيم', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 25, parentId: alHusayn.id, childCount: 0, biography: 'A massive tribal confederation in Syria (Homs, Golan, Deraa) and beyond. By oral tradition, they claim Ashraf descent from Al-Husayn.', biographyAr: 'تحالف قبلي ضخم في سوريا (حمص والجولان ودرعا) وغيرها. بالتواتر الشفهي ينسبون أنفسهم للأشراف من نسل الحسين.', birthPlace: 'الجولان / حوران', era: 'العصور الحديثة', latitude: 33.1, longitude: 35.8 });
        levantTribes.push({ name: 'Al-Zoubi', nameAr: 'الزعبي', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 25, parentId: alHusayn.id, childCount: 0, biography: 'A massive agricultural clan centered in northern Jordan (Ramtha/Irbid) and southern Syria (Daraa/Hauran). They traditionally claim descent from Al-Husayn.', biographyAr: 'عشيرة زراعية ضخمة تتمركز في شمال الأردن (الرمثا/إربد) وجنوب سوريا (حوران). ينسبون أنفسهم تقليدياً إلى الحسين بن علي.', birthPlace: 'حوران (سوريا/الأردن)', era: 'العصور الحديثة', latitude: 32.6, longitude: 36.0 });
        levantTribes.push({ name: 'Hadidiyyin', nameAr: 'الحديديين', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 25, parentId: alHusayn.id, childCount: 0, biography: 'A large tribe predominantly in the Hama and Aleppo regions of Syria, claiming descent from the Prophet Muhammad.', biographyAr: 'قبيلة كبيرة تتواجد بكثافة في مناطق حماة وحلب في سوريا، وينسبون أنفسهم لآل بيت النبي محمد ﷺ.', birthPlace: 'حماة وحلب، سوريا', era: 'العصور الحديثة', latitude: 35.5, longitude: 37.0 });
    }
    if (makhzumId) {
        levantTribes.push({ name: 'Bani Khalid', nameAr: 'بني خالد', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 18, parentId: makhzumId, childCount: 0, biography: 'A major tribe in the Badia (Jordanian Mafraq & Syria). They traditionally claim descent from Khalid ibn al-Walid of Banu Makhzum, though contested by some historians.', biographyAr: 'قبيلة رئيسية في البادية (المفرق الأردنية وسوريا). ينسبون أنفسهم تقليدياً لخالد بن الوليد المخزومي، رغم اختلاف بعض المؤرخين.', birthPlace: 'بادية الشام', era: 'العصور الحديثة', latitude: 32.4, longitude: 36.3 });
    }
    if (hawazinId) {
        levantTribes.push({ name: 'Uqayl (Uqaylids)', nameAr: 'بنو عقيل', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: hawazinId, childCount: 0, biography: 'An ancient and historically powerful tribal group from Banu \'Amir (Hawazin). They migrated to the Euphrates and Levant, establishing the Shia Uqaylid dynasty.', biographyAr: 'مجموعة قبلية عريقة قوية من بني عامر (هوازن). هاجروا للفرات والشام وأسسوا الأسرة العقيلية (الدولة العُقيلية).', birthPlace: 'الجزيرة الفراتية، سوريا', era: 'العصور الوسطى', latitude: 36.0, longitude: 39.5 });
    }
    if (qudaaId) {
        ...
        levantTribes.push({ name: 'Al-Hajaya', nameAr: 'الحجايا', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: qudaaId, childCount: 0, biography: 'A significant tribal confederation in southern Jordan (especially Karak and Tafilah), historically branched off from the Huwaitat.', biographyAr: 'تحالف قبلي هام في جنوب الأردن (تحديداً الكرك والطفيلة)، تفرّع تاريخياً من قبيلة الحويطات.', birthPlace: 'الكرك / الطفيلة، الأردن', era: 'العصور الحديثة', latitude: 31.0, longitude: 35.7 });
    }
    if (madhhijId) {
        ...
        levantTribes.push({ name: 'Al-Jubur', nameAr: 'الجبور', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: madhhijId, childCount: 0, biography: 'The largest tribe in the Syrian Jazira (Deir ez-Zor, Hasakah). Descends from Zubayd of Madhhij.', biographyAr: 'أكبر وأبرز قبائل الجزيرة السورية (دير الزور، الحسكة). تنحدر من زبيد من مذحج.', birthPlace: 'الحسكة، سوريا', era: 'العصور الحديثة', latitude: 36.5, longitude: 40.7 });
        levantTribes.push({ name: 'Al-Weldi', nameAr: 'الولدة', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: madhhijId, childCount: 0, biography: 'A key tribe along the Syrian Euphrates (Raqqa). Forms a core part of the larger Sha\'ban confederation tracing back to Zubayd/Madhhij.', biographyAr: 'من القبائل الهامة على ضفاف الفرات (الرقة). تشكل جزءاً من تحالف قبائل شعبان الذي يعود في أصوله إلى زبيد من مذحج.', birthPlace: 'الرقة، سوريا', era: 'العصور الحديثة', latitude: 35.9, longitude: 38.5 });
    }
    if (judhamId) {
        levantTribes.push({ name: 'Abbad', nameAr: 'عباد', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: judhamId, childCount: 0, biography: 'The second largest tribe in Jordan, historically rooted in the Balqa region. They trace back to Qahtan via Judham. Known for their powerful historical presence from the Zarqa River to Wadi Shuaib.', biographyAr: 'ثاني أكبر قبيلة في الأردن، جذورها التاريخية في منطقة البلقاء. يعود نسبهم إلى قحطان عبر جذام. عُرفوا بحضورهم التاريخي القوي من نهر الزرقاء إلى وادي شعيب.', birthPlace: 'البلقاء، الأردن', era: 'العصور الحديثة', latitude: 31.9, longitude: 35.7 });
        levantTribes.push({ name: 'Al-Manasir', nameAr: 'المناصير', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 11, parentId: judhamId, childCount: 0, biography: 'A major and influential sub-branch (or independent confederate) of the Abbad confederation deeply rooted in the Balqa region and Amman.', biographyAr: 'فرع (أو تحالف) رئيسي ومؤثر من قبيلة عباد، مترسخ بعمق في منطقة البلقاء وعمّان.', birthPlace: 'البلقاء، الأردن', era: 'العصور الحديثة', latitude: 31.95, longitude: 35.85 });
        levantTribes.push({ name: 'Bani Hamida', nameAr: 'بني حميدة', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: judhamId, childCount: 0, biography: 'A massive tribe in central Jordan, particularly in Madaba, Karak, and Tafileh. They are rooted in the ancient lineage of Judham. Played significant roles in historical uprisings against Ottoman authority.', biographyAr: 'قبيلة ضخمة في وسط وجنوب الأردن، وخاصة في مادبا والكرك والطفيلة. جذورهم في نسب جذام العريق. لعبوا أدواراً بارزة في الانتفاضات التاريخية ضد السلطة العثمانية.', birthPlace: 'مادبا والكرك', era: 'العصور الحديثة', latitude: 31.4, longitude: 35.7 });
        levantTribes.push({ name: 'Al-Ajarma', nameAr: 'العجارمة', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: judhamId, childCount: 0, biography: 'A significant Bedouin tribe located primarily in the Balqa and Madaba governorates of Jordan. They typically share ancestral lineage links with Judham.', biographyAr: 'تجمع قبلي وعشائري هام يقطن أساساً في محافظتي البلقاء ومادبا الأردنيتين. ويرتبط نسبهم تاريخياً إلى جذام.', birthPlace: 'البلقاء ومادبا', era: 'العصور الحديثة', latitude: 31.8, longitude: 35.8 });
        levantTribes.push({ name: 'Bani Atiyeh', nameAr: 'بني عطية', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: judhamId, childCount: 0, biography: 'A prominent tribe with branches extending across the Hijaz region, southern Jordan, and Egypt. Known for their nomadic and settled presence in Karak.', biographyAr: 'قبيلة بارزة تمتد فروعها عبر الحجاز وجنوب الأردن ومصر. عُرفت بتواجدها البدوي والمستقر في الكرك.', birthPlace: 'جنوب الأردن / الحجاز', era: 'العصور الحديثة', latitude: 30.5, longitude: 35.5 });
    }
    if (qahtan.id) {
        levantTribes.push({ name: 'Al-Masa\'eed', nameAr: 'المساعيد', type: NodeType.TRIBE, status: NodeStatus.PUBLISHED, generationDepth: 10, parentId: qahtan.id, childCount: 0, biography: 'A prominent Bedouin tribe in the northern Badia (e.g., Umm al-Jimal, Mafraq). Traditionally claim descent from Lakhm or Qahtani lineages.', biographyAr: 'قبيلة بدوية بارزة في البادية الشمالية الأردنية (مثل أم الجمال والمفرق). ينسبون أنفسهم تقليدياً لخم أو لنسب قحطاني.', birthPlace: 'المفرق، الأردن', era: 'العصور الحديثة', latitude: 32.32, longitude: 36.37 });
    }
    if (qaysAylan.id) {
